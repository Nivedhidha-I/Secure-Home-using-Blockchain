...
This project demonstrates a proof-of-concept for securing an Internet of Things (IoT) environment by integrating a custom-built blockchain and a smart contract. The blockchain is used as a tamper-proof ledger to immutably record critical IoT sensor data (e.g., from heart rate monitors or environmental sensors), ensuring data integrity. The smart contract provides decentralized governance, enforcing logical rules for device control (e.g., turning a device ON/OFF) based on the recorded sensor inputs.
...
* Data Layer: Custom-built Blockchain (Python/Flask) implementing basic Proof-of-Work (PoW) consensus.

* Control Layer: Solidity Smart Contract (device contract) defining physical device states and state transition logic (e.g., shouldOnFAN modifier).
...
* Decentralized Ledger Technology (DLT): Gained practical experience designing and implementing a functional blockchain, including hashing, immutability, and basic consensus.

* Smart Contract Development: Proficiency in writing, testing, and applying Solidity to enforce physical world rules through code.

* IoT Security Architecture: Demonstrated a strategic understanding of securing IoT data streams against tampering and single points of failure.
